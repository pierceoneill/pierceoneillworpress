...
class Echo_RSS_Feed_Display 
{
	...
	private function get_title($item) {
		if ($this->settings['title']) {
		    $output = '<a class="echo_rss_title"' . $this->get_link_target() . ' href="' . esc_url($item->permalink) . '"' .
	...
	private function get_thumbnail($item) {
		if (property_exists($item, 'thumbnail') && $this->settings['thumbnail'] && strlen($item->thumbnail) > 0) {
			$output = '<a class="echo_rss_image"' . $this->get_thumbnail_inline_styles() . $this->get_link_target()  . $this->get_link_dofollow() . ' href="' . esc_url($item->permalink) . '">';
