...
class NILOS_El_Woocommerce
{
    ...
    public static function nilos_woo_add_to_cart( $args = array() ) {
        global $product;
            ...
            if ( $product ) {
             ...
             if( $product->is_type( 'simple' ) ){
                $btntext = esc_html__("Add to Cart",'nilos');
             } elseif( $product->is_type( 'variable' ) ){
                $btntext = esc_html__("Select Options",'nilos');
             } elseif( $product->is_type( 'external' ) ){
                $btntext = esc_html__("Buy Now",'nilos');
             } elseif( $product->is_type( 'grouped' ) ){
                $btntext = esc_html__("View Products",'nilos');
             }
             else{
                $btntext = "Add to Cart";
             } 
    ...
    // quick view
    public static function gnilos_woosq_button_html( $ounilosut , $prodid ) {
        return $ounilosut = '<a href="#" class="icon-btn woosq-btn woosq-btn-' . esc_attr( $prodid ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $prodid ) . '" data-effect="mfp-3d-unfold"><svg width="19" height="16" viewBox="0 0 19 16" fill="none" xmlns="htnilos://www.w3.org/2000/svg">
    ...
    public static function add_to_cart_button($product_id)
    {
        ...
        if ($product) {
            ...
            return sprintf('<a rel="nofollow" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s add_to_cart_button nilos-btn"><i class="fal fa-shopping-cart"></i> Add To Cart</a>',
                esc_url($product->add_to_cart_url()),
